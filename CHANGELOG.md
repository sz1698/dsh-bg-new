...
- 重新核对全部对外文档里的路径、路由、命名空间与包名，删掉与发布无关的开发过程记录
  （构建笔记等只留在本地，不进仓库）。
- README 顶部加界面预览图 `assets/preview.png`，并把 `assets/**` 加进 `package.json`
  的 `files`（否则将来发 npm 时 README 里的图是裂的）；Gitee 镜像链接指向
  `tree/main`（该仓库默认分支是空的 `master`，不指分支会看到空仓库）。

### 界面修正

- **页签底色改为半透明**：药丸式分段控件的底槽与选中滑块改用按文字方案取值的
  半透明 tint（浅字方案 `rgba(255,255,255,0.14)` / `0.28`，深字方案
  `rgba(15,17,21,0.10)` / `0.22`，未设背景时中性灰）。此前底槽吃主题的
  `--dsw-specific-selector`（不透明静态色）、滑块吃 `--dsw-alias-bg-layer-1`
  （本插件调色板里是 `rgb(10 13 18 / 0.96)`，近黑），在壁纸上就是一条实心黑条。
- **预设页签名从「系统」改回「推荐」**（英文 `Presets`）。
- **画布底色改为确定值**：引擎把该方案 `--dsw-alias-bg-layer-1` 的实心形式写到
  `html` 上（浅字方案 `rgb(10 13 18)`、深字方案 `rgb(250 251 253)`；`mode=off`
  时不写、完整还原）。此前图层以下没有底色，而 `--dsw-alias-bg-base` 被置
  `transparent` —— 于是媒体不透明度 <1 的混合区与 =0 的完全透明区露出的是**运行
  环境自己的颜色**：网页端浏览器画布是白的（泛白），桌面窗口底色是黑的，同一个设置
  两端观感不同。现在图层仍在 `html` 之上、壁纸照常透出，而半透明/全透明处两端一致。
- **侧栏「壁纸」按钮补悬浮底色**：与它下面的 Settings 控件同一枚
  `--dsw-alias-interactive-bg-hover`（浅字方案 = `rgba(255,255,255,0.10)`，即
  白色透明）。按钮底色是**内联样式**、层级高于样式表，所以由抽屉布局样式表补一条
  `:hover` 规则并加 `!important` 才生效。
