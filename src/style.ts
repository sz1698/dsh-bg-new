/**
 * dsh-bg-style —— host 样式注入插件（v0.4 起为 no-op 占位）。
 *
 * 背景：v0.3.x 里本插件监听 webserver/index-inject 注入一段固定 center/cover 的
 * body 媒体 CSS（无 pos/opacity/fit 支持），与客户端渲染引擎（<style id=dsh-bg-style>
 * + [data-dsh-bg-layer]）**同时**生效 → 桌面实测出现图片"重叠"（双渲染器都画了
...
export const name = 'dsh-bg-style'
