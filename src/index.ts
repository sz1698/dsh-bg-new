/**
 * dsh-bg-switch —— 单一 host loader 入口（R3 布局；v0.3 追加 media 嵌套插件；
 * v0.4 移除 style 嵌套）。
 *
 * 为什么收敛成一个入口：
 * dsh-client-modules 的扫描按 package.json 的 name 去重；一个包被多个
 * 活动的 loader 源同时命中时（本仓库声明 dsh.client 之后），激活即抛
 * "package dsh-bg-switch resolves from multiple active Loader sources:
 * remove one entry"。因此本包必须恰好一个 loader 行。
 *
 * 本文件把工具（tool.ts：bg_apply）与本地媒体路由（media.ts：/dsh-bg-media
 * 上传+Range 伺服）作为嵌套插件挂载；另在父 fiber 上安装 settings 命名空间
 * 'dsh-bg' 的注册（src/bg-settings.ts：随 settings 服务可用性延迟注册，官方同款
 * ctx.inject 姿势，参照 packages/client/ui-theme/src/index.ts:36-43）。命名空间
 * 注册只做一次，tool/media 都经 bg-settings.ts 的模块级句柄读写，避免重复注册
 * （settings 服务抛 duplicate）与激活时序问题（eager 注册可能在 provider
 ...
 *
 * media.ts 声明 inject:['webServer']：无 webServer 的 profile（headless）下该
 * 嵌套插件保持未激活、无副作用，不影响工具与命名空间。
 */
