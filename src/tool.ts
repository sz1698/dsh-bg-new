...
function normalizeMediaUrlValue(raw: string): string {
  const value = raw.trim()
 ...
 * 扩展名须在 kind 对应允许表、文件可读、大小 ≤ 上限（用 config）。
 * 返回绝对路径；超限/格式错抛中文 Error。
 */
function validateLocalMediaFile(path: string, kind: 'image' | 'video', cfg: BgConfig): string {
  ...
  if (size > limitMB * 1024 * 1024) {
    throw new Error(`bg_apply: 本地${kind === 'image' ? '图片' : '视频'} ${size} 字节超过上限 ${limitMB}MB`)
  }
  return path
}
...
export function executeBgApply(args: BgApplyArgs, cfg: BgConfig): BgState {
  ...
  switch (modeRaw) {
    ...
    case 'image': {
      ...
      const value = normalizeMediaUrlValue(String(args.value ?? ''))
    ...
    case 'off':
      // v0.4：整命名空间重置 —— 运行时字段全部回默认（config 默认 / 1 / 50 / 50 / 1）
      return {
        mode: 'off',
        value: '',
        mediaKey: '',
...
export function apply(ctx: Context): void {
  ctx.tools.register(defineTool({
    ...
    description: '更换 DeepSeek Harness 网页界面的背景。当用户说想换背景、换配色、换皮肤、放壁纸图片、放背景视频时使用。' +
      ...
      'mode=off 恢复默认。fit/textScheme/opacity/posX/posY 可选（opacity/posX/posY 仅 image/video：opacity=媒体不透明度 0–1 小数或 0–100 百分数；posX/posY=焦点定位百分比 0–100，50/50=居中）。' +
    ...
    parameters: {
      ...
      value: {
        type: 'string',
        description: 'color: CSS 颜色如 #1e2a78；gradient: 完整 CSS 渐变；image/video: http(s) URL',
      },
      file: {
        type: 'string',
        description: '本地文件绝对路径（image/video 通用；登记伺服不内联：校验后生成 mediaKey 由插件媒体路由 /dsh-bg-media/<key> 伺服，image ≤maxImageMB / video ≤maxVideoMB）',
      },
      fit: {
        type: 'string',
