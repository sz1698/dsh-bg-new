...
function inheritLegacyBgNamespace(settingsCtx, config, scope) {
	for (const legacyNs of BG_NAMESPACE_LEGACY) try {
		...
		scope.update(patch).then(() => settingsCtx.logger.info(`dsh-bg-new: inherited settings from legacy namespace "${legacyNs}"`), (error) => settingsCtx.logger.warn(`dsh-bg-new: could not inherit settings from "${legacyNs}": ${String(error)}`));
	} catch (error) {
