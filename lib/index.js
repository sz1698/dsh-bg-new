...
*/
const BG_HOME_DIR_LEGACY = ["dsh-bg", "dsh-bg-switch"];
/**
...
function bgHomeDir() {
	...
	try {
		if (!existsSync(next)) for (const legacyName of BG_HOME_DIR_LEGACY) {
...
function readRaw() {
	try {
		...
		return {
			raw: void 0,
			warnings: [`dsh-bg-new: 读取 config.json 失败（${error instanceof Error ? error.message : String(error)}），使用内置默认配置`],
			source: "default"
		};
	}
}
...
* 避免升级后背景悄悄回到默认。
*/
const BG_NAMESPACE_LEGACY = ["dsh-bg"];
/**
...
*/
function inheritLegacyBgNamespace(settingsCtx, config, scope) {
	for (const legacyNs of BG_NAMESPACE_LEGACY) try {
		...
		if (hasUserFields(described.find((row) => row.ns === "dsh-bg-new")?.user)) continue;
		...
		scope.update(patch).then(() => logSafe(settingsCtx, "info", `dsh-bg-new: inherited settings from legacy namespace "${legacyNs}"`), (error) => logSafe(settingsCtx, "warn", `dsh-bg-new: could not inherit settings from "${legacyNs}": ${String(error)}`));
	} catch (error) {
...
*/
function logSafe(settingsCtx, level, message) {
	try {
		settingsCtx.logger[level](message);
	} catch {}
}
/** 读 provider 的 settings 文档路径（非文件型 provider 返回 undefined）。 */
function readSettingsDocumentPath(settingsCtx) {
	try {
		const path = settingsCtx.settings.documentPath;
...
*/
function settingsDocumentHasSection(documentPath, ns) {
	if (documentPath === void 0) return false;
	try {
		return readFileSync(documentPath, "utf8").split(/\r?\n/).some((line) => line.startsWith(`${ns}:`));
	} catch {
		return true;
	}
}
/** 该 user 层是否真的写了东西（`{}` / undefined / 非对象都算没写）。 */
function hasUserFields(user) {
...
*/
function pickInheritableFields(user) {
	if (typeof user !== "object" || user === null || Array.isArray(user)) return void 0;
	const source = user;
	const patch = {};
	for (const field of BG_INHERITABLE_FIELDS) if (source[field] !== void 0) patch[field] = source[field];
...
*/
const name$1 = "dsh-bg-new-media";
const inject$1 = ["webServer"];
/** 本插件持有的 webServer 前缀（客户端用它拼 src / 上传 URL）。 */
const MEDIA_PATH_PREFIX = "/dsh-bg-new-media";
...
function mediaKeyOf(req) {
	try {
		...
		return decodeURIComponent(pathname.slice(18));
	} catch {
		return "";
	}
}
...
function validateUpload(kindRaw, extRaw, size, cfg) {
	...
	if (kind === null) return {
		ok: false,
		status: 400,
		message: "dsh-bg-new-media: 参数 kind 需是 image 或 video"
	};
	const ext = (extRaw ?? "").trim().toLowerCase().replace(/^\.+/, "");
	if (!/^[a-z0-9]{1,12}$/.test(ext)) return {
		ok: false,
		status: 400,
		message: `dsh-bg-new-media: 缺少或非法的扩展名参数 ext（如 png / mp4），收到：${JSON.stringify(extRaw ?? "")}`
	};
	const allowed = kind === "image" ? cfg.imageExt : cfg.videoExt;
	if (!allowed.includes(ext)) return {
		ok: false,
		status: 400,
		message: `dsh-bg-new-media: 不支持的${kind === "image" ? "图片" : "视频"}类型 .${ext}（允许 ${allowed.join(" / ")}）`
	};
	...
	if (size !== null && Number.isFinite(size) && size > limitBytes) return {
		ok: false,
		status: 400,
		message: `dsh-bg-new-media: 文件 ${size} 字节超过上限 ${limitMB}MB（kind=image）`
	};
	return {
		ok: true,
		limitBytes
...
async function handleUpload(req, res) {
	...
	try {
		...
		jsonResponse(res, 400, {
			ok: false,
			message: `dsh-bg-new-media: 请求参数解析失败（${error instanceof Error ? error.message : String(error)}）`
		});
		req.resume();
		return;
	}
	...
	try {
		...
		jsonResponse(res, 500, {
			ok: false,
			message: `dsh-bg-new-media: 创建媒体目录失败（${error instanceof Error ? error.message : String(error)}）`
		});
		req.resume();
		return;
	}
	...
	out.on("error", () => {
		...
		jsonResponse(res, 400, {
			ok: false,
			message: "dsh-bg-new-media: 写入媒体目录失败"
		});
		req.resume();
	});
	req.on("error", () => {
		...
		jsonResponse(res, 400, {
			ok: false,
			message: "dsh-bg-new-media: 读取上传流失败"
		});
	});
	let received = 0;
	req.on("data", (chunk) => {
		...
		if (received > limitBytes) {
			...
			jsonResponse(res, 400, {
				ok: false,
				message: `dsh-bg-new-media: 上传超过上限 ${Math.round(limitBytes / 1024 / 1024)}MB（kind=image）`
			});
			req.resume();
			return;
		}
	...
	req.on("end", () => {
		...
		out.end(() => {
			...
			try {
				...
				jsonResponse(res, 500, {
					ok: false,
					message: `dsh-bg-new-media: 落盘失败（${error instanceof Error ? error.message : String(error)}）`
				});
				return;
			}
...
async function handleMedia(req, res) {
	...
	if (req.method === "POST" && pathname === "/dsh-bg-new-media/upload") {
