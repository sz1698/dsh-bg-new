window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		* @returns 不透明的 CSS 颜色。
		*/
		function baseSurfaceForTextScheme(scheme) {
		...
		*/
		function tabTintForTextScheme(scheme) {
			if (scheme === "dark") return {
				track: "rgba(15,17,21,0.10)",
				thumb: "rgba(15,17,21,0.22)"
			};
			...
			return {
				track: "rgba(127,127,127,0.14)",
				thumb: "rgba(127,127,127,0.26)"
			};
		}
		/**
		...
		*/
		const GLASS_BACKDROP_SELECTOR = "[data-dsh-bg-new-drawer], div[role=\"dialog\"][aria-modal=\"true\"], [data-composer-card]";
		/**
		...
		*/
		async function bgUploadLocalFile(file, kind, cfg) {
			...
			try {
				...
				return {
					ok: false,
					message: "dsh-bg-new-media: 上传失败（无法连接媒体服务）"
				};
			}
			let payload = {};
			try {
			...
			return {
				ok: false,
				message: typeof payload.message === "string" && payload.message !== "" ? payload.message : "dsh-bg-new-media: 上传失败（服务端拒绝）"
			};
		}
		const inject = [
			"slots",
		...
		function apply(ctx) {
			...
			const t = ctx.locale ? ctx.locale.bind("settings.dsh-bg-new") : void 0;
			...
			*/
			...
			const minimapBlock = mapEnabled ? (0, react_jsx_runtime.jsx)("div", {
				...
				children: [(0, react_jsx_runtime.jsx)("div", {
					...
					"data-dsh-bg-new-map-outside": mapOutside ? "" : void 0,
					...
					style: {
						...
						touchAction: "none",
						userSelect: "none",
						animation: mapOutside ? "dsh-bg-new-minimap-outside 0.9s ease-in-out infinite" : void 0
					},
			...
			const currentLabel = snap.mode === "off" || snap.mode === "" ? text("currentOff") : `${text("current")}: ${snap.mode} · ${(snap.value !== "" ? snap.value : snap.mediaKey !== "" ? `/dsh-bg-new-media/${snap.mediaKey}` : "").slice(0, 120)}`;
...
`;
		...
		*/
		...
		exports.baseSurfaceForTextScheme = baseSurfaceForTextScheme;
		...
		exports.tabTintForTextScheme = tabTintForTextScheme;
